...
When we investigate the categorical variables, we see that some have a larger difference between time until recurrence than others.  When we first investigate menopause, we see that recurring patients are very similar in their recurrence time.  However, non-recurring patients have a slightly lower median with a similar distribution.  A very similar pattern is observed for chemotherapy but the non-recurring patients had a slightly higher time before leaving the study.  While size, having three levels, needs to be interpreted slightly differently.  With patients that relapsed into cancer, the larger cancer cells certainly caused it to happen sooner.  This makes intuitive sense that more severe cancer would have more long term health risks.  Among patients that do not relapse, only the extremely large cancer patients had a much lower recurrence time.  While small and medium patients had a very similar response.  Grade had a very similar response among relapsing and non-relapsing patients.  Patients given hormonal therapy seem to have a smaller tail among relapsing patients, meaning those who relapse are more likely to do it soon after treatment.  Non-relapsing patients that had hormonal therapy also appear to be censored more quickly than those who do not receive the treatment.

```{r boxPlots, include=TRUE, message = FALSE}
library(ggplot2)
...
Among continuous variables, we first investigated age.  We plotted a both the lowess curve and the linear model to see if they match.  If so, they most likely have data with a limited skew and do not require transformation for modeling.  We first evaluated age and there was a difference between the linear and lowess curves.  To adjust for this, a log transform was performed and the fit generally improved for most of the data.  Next, we investigated the relationship between er and relapse time.  Here we see a large deviation which was corrected for again with a log transform.  Finally, we evaluated pgr and it had a good fit.



```{r scatterPlots, include=TRUE, message=FALSE}
...
# log(age)
ggplot(data, aes(x = log(age), y = dtime, color = death)) +
  geom_point() + geom_smooth(formula='y ~ x', method="loess",size=2, se=FALSE) + 
  geom_smooth(formula='y ~ x', method="lm",size=2, se=FALSE) + scale_color_manual(values=c("darkgreen", "red"))
...
The variable size is a categorical variable with 3 levels so we recode the levels 20-50 and >50 as binary variables and take the level <=20 as reference level.
We then estimate the full model only excluding rtime and recur variables since they are not the chosen event of interest.
...
For model selection with all covariates, we first fit a full model to apply backward selection procedure. By excluding the covariate with the largest p-value in each iteration, we ended up with the model "Surv(dtime,as.numeric(death))~meno+hormon+size+ age+grade+nodes+pgr" where all covariates are significant. Then we also run stepwise AIC selection in both direction and it resulted the model with size, age, 
grade, nodes, pgr as variables. To decide which criteria should be used to choose the model, we approach from 2 different points. From a logical point of view, the model including hormonal treatment is reasonable since it might have a direct influence on survival time. If we compare AIC of both models, we see that there is no hugh difference (18543 and 18546). So we decide to go with the model selected based on p-value since it includes hormon in exchange of acceptable increase in AIC.
...
row.names(fit.ph.table)[9]<-"size>50 vs size20-50"
colnames(fit.ph.table)<-c("HR","95% CI Lower Bound","95% CI Upper Bound")
kable(fit.ph.table, 
...
table <- tidyModel
table <- cbind(tidyModel, confint(bestModel), linCoeffs)
table
```


When we look at our categorical variable, chemo, it is highly significant and the confidence interval confirms this as it does not contain 0.
