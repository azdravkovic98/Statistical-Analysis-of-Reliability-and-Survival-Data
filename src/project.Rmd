...
**ADD EXPLANATION ON NA VALUES, SANDRA**
...
The variable \textit{size} is a categorical variable with 3 levels. We recode the levels Size ($20-50$) and Size ($>50$) as binary variables, and take the level Size ($\leq20$) as reference level. The same holds for the variable \textit{grade}, which has 2 levels. The level \textit{Grade = 3} is coded as a new binary variable, with \textit{Grade = 2} being the reference level.
...
Note that hazard ratios are assumed to be constant over time to estimate the PH model in the first place. With the purpose of checking the validity of constant proportional hazard assumption , we run a statistical test based on Schoenfeld residuals for each covariate included in the fitted model with the null hypothesis of residuals being independent from time. From the Table \@ref(tab:PH-test), one can see that test is statistically highly significant for the covariates *pgr* and *age* with nodes and menopause being a borderline case. It shows that relative risks for categorical variables are constant over time while null hypothesis of constant hazard ratio is rejected for continuous variables. It could be claimed that the violation by *age* is not entirely unexpected because the difference between the instantaneous risks of dying of two subjects with consecutive ages at a given time would logically and naturally increase as age increases even for healthy subjects without cancer and so does the proportion of these risks, which is measured by hazard ratio. Also the global test is highly significant with p-value~0 and the null hypothesis is rejected. It can be concluded that proportional hazard assumption of constant hazard ratio over time does not hold for the model since there is significant dependency between Schoenfeld residuals and time.

As a solution to this problem, we propose a stratified PH model. First, the covariate pgr is stratified since its chi square statistics is the highest after it is binned with equally long (1500) bins. Then we run the test again for stratified model but the test output shows that PH assumption is still violated by the covariate *age* with a p-value 0,0022. Then we stratify it after binning it with the bins size of 20 and estimate the PH model with these stratified variables. When the test for proportional hazard assumption run again, test statistics for all covariates and global test result p-values all above 0,05 as seen in the Table. Thus, Constant PH assumption holds for the proposed stratified model.
...
rownames(test2) <- c("Meno",
                     ...
                     "Nodes",
                     "GLOBAL")
kbl(round(test2, 4), booktabs = T, escape = FALSE, caption = "Proportional Hazard Test") %>%
  kable_styling(latex_options = c("striped", "hold_position"))
 
```
Lastly, we check again the significance level of covariates in the final model we propose to confirm whether the model selection performed at the beginning is still valid. As seen in the table, all variables are still highly significant for the model except the *hormon* but we decided to keep it in the model since its p-value of 0,0523 is on the borderline.
