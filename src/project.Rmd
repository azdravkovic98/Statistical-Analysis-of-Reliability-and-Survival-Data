...
kbl(fit_ph_final$coefficients, 
    escape = FALSE, booktabs = TRUE,
    caption = "Estimated Coefficients of Covariates Under the Selected Model",
     col.names = "$\\beta$" ) %>% kable_styling(font_size = 15, latex_options = c("striped", "hold_position"))
```
...
kbl(test, booktabs = T, caption = "Proportional Hazard Test") %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```
```{r eval=FALSE, fig.show="hold", message=FALSE, warning=FALSE, include=FALSE, out.width=}
library(survival)
library(cowplot)
