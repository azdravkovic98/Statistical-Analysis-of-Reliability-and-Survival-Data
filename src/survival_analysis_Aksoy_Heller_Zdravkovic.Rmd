...
When investigating continuous variables, we plotted both the LOWESS curve and the linear model to see if they match.  If so, they most likely have data with a limited skew and do not require transformation for modeling. First, we investigated the relationship between \textit{er} and death time.  Here we see a large deviation which was corrected for with a log transformation. Covariate \textit{pgr} performs similarly to \textit{er}.
Finally, we evaluated \textit{age} and there was not a difference between the linear and LOWESS curves.
For these reasons, models with logarithmic transformations of covariates *er* and *pgr* will be evaluated in Section \@ref(modeling).
...
# log(pgr)
ggplot(data, aes(x = log(pgr), y = dtime, color = death)) +
  geom_point() + geom_smooth(formula='y ~ x', method="loess",size=2, se=FALSE) + 
  geom_smooth(formula='y ~ x', method="lm",size=2, se=FALSE) +
  theme_minimal()+
  scale_color_manual(values=c("darkolivegreen", "darkred"))
...
```{r interaction-plot, echo=FALSE, message=FALSE, warning=FALSE, fig.align='center', out.width="70%"}
# interaction chemo:meno
interaction.plot(x.factor=data$meno, 
                 trace.factor=data$chemo, 
                 response=data[,"dtime"], 
                 fun=mean, 
                 type="b", 
                 legend=T,
                 xlab="Hormon",
                 ylab="Death Time Means",
                 main="Interaction Meno:Chemo",
                 trace.label="Chemo",
                 pch=c(1,19), 
                 col = "darkolivegreen")
```

When evaluating the interaction plot, we see that the lines are not parallel. This indicates there is an interaction effect between the two, i.e. the death time of menopausal patients is affected by chemotherapy treatment. This is why model with this interaction will be evaluated in Section \@ref(modeling).
...
##fitting full model
data$log.pgr <- log(data$pgr + 2)
data$log.er <- log(data$er + 2)
fit_ph1 <-
  coxph(
    Surv(dtime, as.numeric(death)) ~ age+meno+size+grade+
                                  log.pgr+log.er+hormon+chemo+meno:chemo,
...
stepAIC <- stepAIC(fit_ph1, scale = 0,direction = "both",trace = 1, keep = NULL, steps = 1000, 
        use.start = FALSE,k = 2)
fit_inter <-   coxph(
    Surv(dtime, as.numeric(death)) ~ age + meno + size + grade + 
    log.pgr + chemo + meno:chemo,
    data = data, ties = "efron"
  )
...
AIC = c(extractAIC(fit_ph_aic)[2], extractAIC(fit_ph3)[2], extractAIC(fit_inter)[2])
names(AIC) = c("AIC Selection", "Backward Selection", "Model with Interaction and Trasformation")
...
kbl(AIC.table, 
    escape = FALSE, booktabs = T,
    caption = "AIC Values for Different Models") %>%
  kable_styling(font_size = 10, latex_options = c("striped", "hold_position"))
```
...
The last model we evaluate contains logarithmic transformations of *pgr* and *er*, as well as an interaction term between *meno* and *chemo*. These covariate modifications were inspired by our exploratory data analysis (see Section \@ref(EDA)). However, it has a worse AIC value than the other two models considered.

We next need to decide between the remaining two models, and we are taking two different approaches to do so. 
...
The estimates of the selected model's covariates are shown in Table \@ref(tab:selected-model-est).


```{r selected-model-est, echo=FALSE, message=FALSE, warning=FALSE}
...
\subsection{Stratified Proportional Hazards Model}

The proportional hazards model assumes that hazard ratios are constant over time. With the purpose of checking the validity of the constant proportional hazard assumption, we perform a statistical test based on Schoenfeld residuals. The null hypothesis for each covariate included in the fitted model is that the residuals are independent from time. 
...
rownames(test) <- c("Meno",
                     ...
                     "Pgr", 
                     "GLOBAL")
kbl(round(test, 4), booktabs = T, escape = FALSE, caption = "Proportional Hazard Test for Non-Stratified PH Model") %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```


From the Table \@ref(tab:PH-test), one can see that test is statistically highly significant for the covariates *pgr* and *age* with *nodes* and *meno* being a borderline case. It shows that relative risks for categorical variables are constant over time, while null hypotheses of constant hazard ratio for continuous variables is rejected. 

Also, the global PH test is highly significant. Hence, there is a significant dependency between Schoenfeld residuals and time, and it can be concluded that the assumption of constant hazard ratios over time does not hold for the model.

As a solution to this problem, we propose a stratified PH model. 
First, the covariate *pgr* is stratified, since its $\chi^2$ statistics is the highest after it is binned with equally sized bins (1500). 
Next, we perform the PH test again for stratified model. However, the test shows that PH assumption is still violated by the covariate *age*. Hence, we stratify by *age* after binning it with size 20 bins and estimate the PH model with these stratified variables. After performing the test for proportional hazard assumption again, we can see that the constant PH assumption holds for the proposed stratified model (see Table \@ref(tab:strat-PH)).


```{r strat-PH, echo=FALSE, message=FALSE, warning=FALSE}
##bin cont. variables
 data$binpgr<-cut(data$pgr,breaks = c(0,1500,3000,4500,6000))
 data$binage<-cut(data$age,breaks = c(20,40,60,80,100))
 ##stratified ph model
 fit5<-coxph(
    Surv(dtime, as.numeric(death)) ~ meno + hormon + as.factor(size) + age +
        as.factor(grade) + nodes + strata(binpgr),
    data = data,
    ties = "efron")
fit6<-coxph(
    Surv(dtime, as.numeric(death)) ~ meno + hormon + as.factor(size) + strata(binage)+  as.factor(grade) + nodes + strata(binpgr),
    data = data,
    ties = "efron")
 
test2 <- cox.zph(fit6)$table
colnames(test2) <- c("$\\chi^2$",
                    "df",
                    "p-value")
rownames(test2) <- c("Meno",
                     "Hormon",
                     "Size",
                     "Grade",
                     "Nodes",
                     "GLOBAL")
kbl(round(test2, 4), booktabs = T, escape = FALSE, caption = "Proportional Hazard Test for Stratified PH Model") %>%
  kable_styling(latex_options = c("striped", "hold_position"))
 
```

Lastly, we check the significance level of covariates in the final model - the stratified PH model. To confirm whether the model selection performed at the beginning of the analysis is still valid, we can look at the significance of the covariates in Table \@ref(tab:signif-strat-ph).

```{r signif-strat-ph, echo=FALSE, message=FALSE, warning=FALSE}
tbl <- anova(fit6)
colnames(tbl) <- c("Log-likelihood",
                    "$\\chi^2$",
                    "df",
                    "p-value")
rownames(tbl) <- c("NULL",
...
kbl(tbl, 
    escape = FALSE, booktabs = T,
    caption = "Chi Square and P-Values for the Stratified PH Model") %>%
  kable_styling(font_size = 10, latex_options = c("striped", "hold_position"))
```


```{r eval=FALSE, fig.show="hold", message=FALSE, warning=FALSE, include=FALSE, out.width="85%"}
library(survival)
library(cowplot)
...
```{r param-AIC, echo=FALSE, message=FALSE, warning=FALSE}
library(KMsurv)
library(survival)
...
log-logistic AFT models. This is done by using AIC selection, and identifying if the additional parameters are worth it.  What we see is that the log-normal model has the best AIC at `r log.normal` (see Table \@ref(tab:param-AIC)).  It should be noted that these AIC values can not be directly compared to the semiparametric values.  
...
Table \@ref(tab:CoefficientComparison) shows point and interval estimates of the coefficient of variables both in the log-normal AFT model, and in the linear representation of the log-normal model.
We are able to estimate the linear coefficients of the log-normal model due to the normality of it's error distribution.


```{r CoefficientComparison, echo=FALSE, message=FALSE, warning=FALSE}
...
When we look at our categorical variable \textit{hormon}, we can see that it is significant. However, the significance is borderline. Next, we evaluate menopausal patients and see that in the parametric model this covariate is not significant. Finally, looking at the size of the tumor and it's grade, they are both highly significant in the log-normal model.
